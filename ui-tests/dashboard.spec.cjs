...
for(const width of [320,390,768,1280]){
  test(`responsive ${width}: both tabs, all cards, audit and overflow`,async({page})=>{
...
test('axe: cash and mile including expanded audit',async({page})=>{
  await page.goto('/');await page.addScriptTag({path:require.resolve('axe-core/axe.min.js')});
  for(const id of ['#tabCash','#tabMile']){
    await page.locator(id).click();await page.locator('.audit summary').first().click();
...
test('keyboard: skip, tab buttons, selection, audit, recommendation target',async({page})=>{
  ...
  const summary=page.locator('.audit summary').first();await summary.focus();await page.keyboard.press('Enter');await expect(page.locator('.audit').first()).toHaveAttribute('open','');
...
test('CSS zoom 200 percent: both tabs, expanded records and no clipping',async({page})=>{
  ...
  for(const id of ['#tabCash','#tabMile']){
    ...
    await page.locator('.audit').evaluateAll(ds=>ds.forEach(d=>d.open=true));
    expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
...
test('mobile: every scenario, headings, expanded records and filter states',async({page})=>{
